...
Les secrets sont lus depuis `/etc/ohana-agent/backup.env`, jamais depuis YAML :
...
Le fichier doit appartenir à `root:ohana-agent` avec le mode `0640`. Les jetons
Home Assistant doivent appartenir à des administrateurs capables d'utiliser le
proxy Supervisor `/api/hassio/backups`.
...
- la destination rclone/iCloud ;
- les noms des variables d'environnement contenant les secrets ;
- le script de préparation NVM de ZWAVE-01.

Vision affiche uniquement si les secrets sont présents sur Agent. Leurs valeurs
ne sont ni lues ni renvoyées par l'API d'administration. Le bouton de test
...
Installer rclone sur INFRA-01, puis créer le remote `icloud` avec le flux SRP et
la validation 2FA. Installer sa configuration dans
`/etc/ohana-agent/rclone.conf`, propriétaire `root:ohana-agent`, mode `0640`.
...
1. Configurer les trois mots de passe et jetons.
2. Configurer rclone et vérifier la lecture du dossier distant.
