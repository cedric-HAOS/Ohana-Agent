"""Tests for generic Home Assistant telemetry freshness checks."""
...
def test_home_assistant_telemetry_accepts_zero_watt_fresh_report() -> None:
    ...
    result = HomeAssistantTelemetryCheck(client=client).check(
        "Cuisine",
        "sensor.shelly_power",
    ...
    assert result.primary.age_seconds == 30.0


def test_home_assistant_telemetry_rejects_stale_report() -> None:
    now = datetime(2026, 7, 27, 18, 0, tzinfo=UTC)
    client = FakeHomeAssistantClient(
        [
            HomeAssistantEntityState(
